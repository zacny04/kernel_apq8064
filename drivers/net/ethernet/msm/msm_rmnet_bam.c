...
static void rmnet_power_suspend(struct power_suspend *h)
{
	if (rmnet0) {
...
static void rmnet_power_resume(struct power_suspend *h)
{
	if (rmnet0) {
		struct rmnet_private *p = netdev_priv(to_net_dev(rmnet0));
...
static struct power_suspend rmnet_power_suspend_handler = {
...
{
	register_power_suspend(&rmnet_power_suspend_handler);
	return 0;
}
